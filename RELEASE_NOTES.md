...
- **NovelAI mode only offers NovelAI's shapes**: with a NovelAI model selected the aspect ratio row shrinks to the five shapes novelai.net itself offers (1:1, 3:2, 16:9, 2:3, 9:16). An imported image or a typed ratio snaps to the nearest of them, and switching a local model over to NovelAI moves a 4:3 or 21:9 onto the nearest NovelAI shape. Local models keep all eight presets.
...
- **Imports never change your resolution**: loading an image into img2img (browse, drop, paste), dropping an image for a whole metadata import, and the NovelAI import modal's Settings pass used to overwrite width and height with the image's size. The resolution is yours: 1024x1024 stays 1024x1024 and the source is fitted to it. Dropping an image onto the Dimensions section itself still applies its size, because that is you asking for exactly that, and inpainting still sizes the canvas to the image so the mask matches. Switching backends no longer recalculates the dimensions either; they only snap to the 64 grid for NovelAI.
- **Imported images set the aspect ratio, not raw pixels**: loading a 1024x1536 image into img2img showed 1024 : 1536 in the aspect ratio inputs instead of 2 : 3. The suggested ratio is now reduced, and on local backends it snaps to the nearest preset within a small tolerance, so a 1216x832 image reads 3 : 2.
