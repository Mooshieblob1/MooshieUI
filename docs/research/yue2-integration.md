...
YuE2 is feasible through native ComfyUI support. A separate inference runtime or
bundled YuE2 custom-node implementation is unnecessary for this integration.
...
Original files also remain in ComfyUI's output directory. Cross-device library
sync, reference-audio transcription/covers (SheetSage2), standalone score-only
generation, and a piano-roll editor are outside this scope.
