...
impl InterrogatorState {
    ...
    pub fn load_session(&mut self) -> Result<(), AppError> {
        ...
        eprintln!("[interrogator] Loading model ({:.0} MB, {} threads)...",
            std::fs::metadata(self.model_path()).map(|m| m.len() as f64 / 1_048_576.0).unwrap_or(0.0),
            thread_count);

        let session = Session::builder()
            .map_err(|e| AppError::InterrogatorError(format!("Failed to create session builder: {}", e)))?
            .with_optimization_level(ort::session::builder::GraphOptimizationLevel::Level1)
            .map_err(|e| AppError::InterrogatorError(format!("Failed to set optimization level: {}", e)))?
            .with_intra_threads(thread_count)
            .map_err(|e| AppError::InterrogatorError(format!("Failed to set thread count: {}", e)))?
            .commit_from_file(&self.model_path())
            .map_err(|e| AppError::InterrogatorError(format!("Failed to load ONNX model: {}", e)))?;
    ...
    pub fn run_inference_from_image(
        ...
        let input_tensor = ort::value::Tensor::from_array((input_shape, input_data))
            .map_err(|e| AppError::InterrogatorError(format!("Failed to create input tensor: {}", e)))?;
        ...
        eprintln!("[interrogator] ONNX inference completed in {:.1?}", t.elapsed());
        ...
        eprintln!("[interrogator] Output '{}': {} probabilities, tag_list: {} tags", output_name, probs.len(), self.tag_list.len());
        if let Some(max_prob) = probs.iter().cloned().reduce(f32::max) {
            let above_threshold = probs.iter().filter(|&&p| p >= general_threshold).count();
            eprintln!("[interrogator] Max prob: {:.4}, above general threshold ({:.2}): {}", max_prob, general_threshold, above_threshold);
        }
...
fn parse_tags_csv(path: &std::path::Path) -> Result<Vec<TagDef>, AppError> {
    ...
    for result in reader.records() {
        let record = result
            .map_err(|e| AppError::InterrogatorError(format!("CSV parse error: {}", e)))?;
