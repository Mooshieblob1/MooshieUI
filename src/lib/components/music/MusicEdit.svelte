<script lang="ts">
  ...
  import type { MusicEditConstraints, MusicEditProposal } from "../../utils/musicEdit.js";
  ...
  onDestroy(() => { sequence++; });
  function preset(kind: "harmony" | "tempo" | "key" | "form" | "translation") {
  ...
  async function propose() {
    ...
    const id = ++sequence, original = cloneMusicParams(baseline), checks = { ...constraints }, intent = brief;
    proposal = null; busy = true; error = "";
    try {
      if (!inspectMusicScore(original.abc).score) throw new Error(locale.t("music.score_invalid"));
      ...
      const result = await promptAssistant.editForMusic({ params: original, brief: intent, constraints: checks });
      if (id !== sequence || owner !== getAuthUser()) return;
  ...
  function apply() {
    ...
    music.params = { ...cloneMusicParams(baseline), abc: proposal.abc, style: proposal.style, lyrics: proposal.lyrics,
      task: "audio", planning: score.voices.Vocal.chords.length ? "full" : "melody", sampling: readMusicSampling(baseline.sampling),
    ...
    music.reviewedCoverAbc = ""; music.saveSettings(); music.view = "generate"; close();
  }
</script>
...
<dialog bind:this={dialog} class="m-auto max-h-[90vh] w-[calc(100%-2rem)] max-w-3xl overflow-y-auto rounded-xl border border-neutral-700 bg-neutral-900 p-5 text-neutral-200 shadow-2xl backdrop:bg-black/70" aria-labelledby="music-composition-title" onclose={close}>
  <form class="space-y-4" onsubmit={event => { event.preventDefault(); void propose(); }}>
    ...
    <div class="flex flex-wrap gap-1">{#each (["harmony", "tempo", "key", "form", "translation"] as const) as kind}<button type="button" class={button} disabled={busy} onclick={() => preset(kind)}>{locale.t(`music.edit_${kind}`)}</button>{/each}</div>
    ...
    <fieldset class="grid gap-2 sm:grid-cols-2" disabled={busy} onchange={() => proposal = null}>
      ...
      <label class="space-y-1 text-xs text-neutral-400"><span>{locale.t("music.edit_melody")}</span><select class={input} bind:value={constraints.melody}><option value="both">{locale.t("music.score_both")}</option><option value="Vocal">{locale.t("music.score_vocal")}</option><option value="Ins">{locale.t("music.score_instrumental")}</option><option value="none">{locale.t("music.edit_unlocked")}</option></select></label>
      {#each (["rhythm", "tempo", "lyrics", "structure"] as const) as key}<label class="touch-target flex items-center gap-2 text-sm"><input type="checkbox" bind:checked={constraints[key]} />{locale.t(`music.edit_keep_${key}`)}</label>{/each}
    </fieldset>
    ...
    {#if proposal}
      <section class="space-y-3 border-t border-neutral-700 pt-3">
        ...
        <MusicNotation abc={proposal.abc} ceiling={baseline?.max_duration} />
        ...
        <button type="button" class={`${button} bg-indigo-500/15`} disabled={music.busy} onclick={apply}>{locale.t("music.edit_apply")}</button>
      </section>
    {/if}
  </form>
</dialog>
